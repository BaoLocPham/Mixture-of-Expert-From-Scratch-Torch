...
`DenseMoe/` — the dense mixture, where every expert runs on every token.
...
makes sparse routing hard is *absent* here, which makes it the right place to
establish what the mixture actually computes — and to measure precisely what
sparsity will later be buying, and at what price.
...
2. **A dissector** (`run_dense_moe.py`) — runs the reference and prints the
...
Sparse top-k routing: what it saves, and what it breaks. The gate stops being a
smooth function of its input the moment you take a top-k of it, and the cost of
repairing that is most of what the rest of the MoE literature is about.
